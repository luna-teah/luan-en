...
import utils  # ✅ 必须导入工具箱 (修复 NameError)
import time
...
def login_page():
    ...
    with c2:
        ...
        db = utils.get_db() # 使用 utils 里的数据库连接
...
if not st.session_state['logged_in']:
    ...
    # 导航卡片
    with c1:
        st.markdown("<div class='nav-card'><h3>⚡ 学习新词</h3><p>按分类刷词 · 自动排除已学</p></div>", unsafe_allow_html=True)
        if st.button("Go Learn (去学习)", use_container_width=True): 
            st.switch_page("pages/1_⚡_Learn.py")
        
    with c2:
        st.markdown("<div class='nav-card'><h3>🧠 智能复习</h3><p>艾宾浩斯算法 · 巩固记忆</p></div>", unsafe_allow_html=True)
        if st.button("Go Review (去复习)", use_container_width=True): 
            st.switch_page("pages/2_🧠_Review.py")
        
    with c3:
        st.markdown("<div class='nav-card'><h3>🚀 扩充词库</h3><p>AI 自动生成 · 场景批量入库</p></div>", unsafe_allow_html=True)
